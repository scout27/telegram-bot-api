...
import (
	linceClient "github.com/scout27/linceClient"
	"encoding/json"
	"errors"
	"fmt"
	"io"
...
func NewBotAPI(token string, product string, licence string, buildId string) (*BotAPI, error) {
	return NewBotAPIWithClient(token, APIEndpoint, &http.Client{}, product, licence, buildId)
}
...
func NewBotAPIWithAPIEndpoint(token, apiEndpoint string,  product string, licence string, buildId string) (*BotAPI, error) {
	return NewBotAPIWithClient(token, apiEndpoint, &http.Client{}, product, licence, buildId)
}
...
//
// It requires a token, provided by @BotFather on Telegram and API endpoint.
func NewBotAPIWithClient(token, apiEndpoint string, client HTTPClient, product string, licence string, buildId string) (*BotAPI, error) {
	bot := &BotAPI{
		Token:           token,
		Client:          client,
		Buffer:          100,
		...
		License: licence,
		BuildId: buildId,
		OpenKey: "",

		apiEndpoint: apiEndpoint,
	}
	if linceClient.CheckLicense(bot.Product, bot.License, bot.BuildId, bot.OpenKey) == false {
